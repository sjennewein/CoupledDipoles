function EIntegrated = EScatteredSolidAngleS(xAtom, yAtom, zAtom, xScreen, yScreen, zScreen, LaserPhase, LaserMag,sinphi,Wp,Wt)
...
EscEl = bsxfun(@times,-3/4.*exp(1i.*phi)./(k.*r),LaserMag);%.*sin(theta);
EIntegrated = permute(sum(sum(bsxfun(@times,sinphi.*Wp.*Wt,EscEl))),[3 2 1]);
