function EIntegrated = EScatteredSolidAngle( dipole, xAtom, yAtom, zAtom, xScreen, yScreen, zScreen, LaserPhase, LaserMag, LaserPolarization, sintheta, Wp, Wt)
...
tmp = permute(dipole,[3 2 1]);
dipoleAlignment=tmp'*LaserPolarization;


if dipoleAlignment == 1
    phi = bsxfun(@plus,k.*r,LaserPhase);
    E = bsxfun(@times,-3/4*exp(1i.*phi)./(k.*r),LaserMag).*sin(theta);
else
    E = -3/4*exp(1i.*k.*r)./(k.*r).*sin(theta);
end
    EIntegrated = permute(sum(sum(bsxfun(@times,sintheta.*Wp.*Wt,E))),[3 2 1]);
end